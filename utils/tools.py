...
def forward_model(model, batch_x, itr, batch_x_mark=None):
    if getattr(model, "requires_time_marks", False):
        return model(batch_x, itr, batch_x_mark)
    return model(batch_x, itr)
...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    with torch.no_grad():
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_pbar, start=1):
            ...
            outputs = forward_model(model, batch_x, itr, batch_x_mark)
            ...
            vali_pbar.set_postfix(loss=f"{float(loss):.5f}")
...
def test(model, test_data, test_loader, args, device, itr, scaler):
    preds = []
    trues = []
    # mases = []
    ...
    with torch.no_grad():
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_pbar, start=1):
            ...
            outputs = forward_model(model, batch_x[:, -args.seq_len:, :], itr, batch_x_mark[:, -args.seq_len:, :])
